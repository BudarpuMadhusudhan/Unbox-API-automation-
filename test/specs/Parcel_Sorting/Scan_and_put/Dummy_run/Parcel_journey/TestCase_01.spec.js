...
describe('Verify if sortId=1 parcel scanned should be mapped to A012 ', async () => {
    /**
     * @type {any}
     */
    var bin_ID
    ...
    it('Executing Update parcel Events API and Validating the response', async () => {
        const csvData =await csvUtil.getDataInCSVByRowNumberAndColumnHeaders('./test_Data/PS_SP_dummy1.csv', [1], ['wbn'])
        const replacements = {
       ...
       expect(response[1].data.sort_id).to.equal('1')
    ...
    it('Executing Robot Events API with BotLoad Event and Validating the response and fetching supplied Bin', async () => {
        ...
        const csvDataBinID =await csvUtil.getDataInCSVByRowNumberAndColumnHeaders('./test_Data/PS_SP_dummy1.csv', [1], ['binid'])
