...
describe('Verify The Item is dumped to Small Bin When the LBH of an item falls under Small Bin Category', async () => {
    /**
     * @type {any}
     */
    var bin_ID
    ...
    it('Executing Get Bin details API and Validating Parcel Dumped and binSize', async () => {
        var queryParam={
            scanned_barcode:"123456654321",
        }
        var response = await apiUtil.getDataWithQueryParam(
            `${process.env.STAGING_URL}${process.env.ECS_API_GATEWAY_PORT}${endPoints.ecsAPIGateway.get_item_details}`,queryParam)
        expect(response[1]).to.not.be.null
        expect(response[1].status).to.be.true
        expect(response[1].StatusCode).to.eq(200)
        expect(response[1].data.more_info.bin_size).to.eq("S")
