...
const myntraPayload=require('../../../../../../payloadData/myntraPayload.data')
...
describe('Verify Putaway success when status type is SUCCESS ', async () => {
    /**
     * @type {any}
     */
    var bin_ID
     ...
     */
    var trace_Id
    var consolidationPacketId=321123 
    it.only('Activating the Station', async () => {
        await genUtil.activateSystemMode("live_run")
        await genUtil.activateSortationMode("order_consolidation")
        await genUtil.activateStation("PS_Station","feeder_1","cognex_right")
    });
    ...
    it('Executing Update parcel Events API and Validating the response', async () => {
       const itemfromCSV=await csvUtil.getDataInCSVByRowNameAndColumnHeaders('./test_Data/Shipment_info.csv',['Putaway'],['ItemBarcode'],'testcase')
       console.log(itemfromCSV[0].ItemBarcode); 
        const replacements = {
            "replace_String": itemfromCSV[0].ItemBarcode,
    ...
    it('Fetching trace id from DB',async()=>{
        trace_Id= await dbUtil.executeQueryInDatabase(`
        select id 
        from sort_service.put_away_queue_status
        where ubr_id='${ubr_id}'`)
        await dbUtil.disconnectDataBaseConnection()
        console.log(trace_Id[0].id);
    })
    it('Executing putaway update API',async()=>{
        const replacement={
            "replacement_ItemBarcode":"10000pw00001",
            "replace_BinBarcode":bin_ID,
            "replace_ConsolidationPacketId":consolidationPacketId, 
            "replace_traceId":trace_Id[0].id,
           }
       const putAwayPayload=await stringUtil.replaceMultiple(myntraPayload.pay,replacement)
       console.log(putAwayPayload);
       const putAwayLoadJSONOBJECT =await stringUtil.stringToJsonObject(putAwayPayload)
       console.log(putAwayLoadJSONOBJECT);
        response=await apiUtil.postData(`${process.env.STAGING_URL}${process.env.N8N_PORT}${myntraEndpoints.putaway}`,putAwayLoadJSONOBJECT)
        console.log(response[1]);
        expect(response[1].message).to.be.equal("Upserted database successfully")
      });
})