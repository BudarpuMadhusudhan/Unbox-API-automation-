...
describe('Verify The Item is dumped to medium Bin When the LBH of an item falls under Medium Bin Category', async () => {
    /**
     * @type {any}
     */
    var bin_ID
    ...
    it('Executing Get Bin details API and Validating Parcel Dumped and binSize', async () => {
        ...
        expect(response[1].status).to.be.true
       // expect(response[1].StatusCode).to.eq(200)
        expect(response[1].data.more_info.bin_size).to.eq("M")
